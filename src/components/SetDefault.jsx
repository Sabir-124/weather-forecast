...
const SetDefaultCity = () => {
  ...
  const { isToggled, setIsToggled, setDefault, cityName, setCityName} = useContext(DefaultContext);
  const { place, setPlace } = useContext(WeatherContext);
  const [defaultResults, setDefaultResults] = useState([]);

  async function handleCityName(evt) {
    setCityName(evt.target.value);
    const data = await searchPlace(evt.target.value);
      ...
      <div className="default-options">
        ...
        <div className="enter-default-place">
          <div className="enter-city">
            <input className='name' placeholder='Enter your default place..' type='text' value={cityName} onChange={handleCityName} />
            <div className={`output-search ${cityName.length === 0 ? 'hide' : 'show'}`}>
        ...
        <div className='set-default'>
          <button className='set-default-button' onClick={hideDefaultOption}>Cancel</button>
